...
for i in range(num_episodes):
    ...
    lidar_data_mapping.update( dynamic_obstacles )
    for j in range(num_steps_per_episode):
        # action = env.action_space.sample()
        action = [ 2, -1, -0.3, 0]  # [vx, vy, vw, fire]; vx: the velocity at which the vehicle moves forward, vy: the velocity at which the vehicle moves to the left, vw: Angular speed of the vehicle counterclockwise rotation, fire: Shoot or not
